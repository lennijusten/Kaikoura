...
class QuakeML:
    ...
    def __init__(self, quakemlcat):
        ...
        ## initialize fields
        quakeml = {'event_id': [], 'event_latitude': [], 'event_longitude': [], 'event_depth': [],
                   'event_origin_time': [], 'event_magnitude': [], \
    ...
    def to_Arrival(self):
        ...
        ArrivalP = Arrival.loc[Arrival['phase'] == 'P', :]
        ArrivalS = Arrival.loc[Arrival['phase'] == 'S', :]

        ArrivalP = ArrivalP.reset_index(drop=True)
        ArrivalS = ArrivalS.reset_index(drop=True)

        ArrivalP['P_time'] = ArrivalP['time']
        ArrivalP['channel'] = [x.replace(x[len(x) - 1], '?') for x in ArrivalP['channel']]
        ArrivalS['S_time'] = ArrivalS['time']
        ArrivalS['channel'] = [x.replace(x[len(x) - 1], '?') for x in ArrivalS['channel']]

        ArrivalP = ArrivalP.drop(columns=['time','phase'])
        ArrivalS = ArrivalS.drop(columns=['time','phase'])

        Arrival = pd.merge(ArrivalP, ArrivalS, how='left', on=['station', 'event_id', 'network', 'channel'])
        # Arrival = Arrival['event_id', 'network', 'station', 'P_time', 'channel_x', 'error_x', 'method_x',
        #                   'S_time', 'channel_y', 'error_y', 'method_y']
        return Arrival
    ...
    def to_Event(self):
        ...
        event_keys = ['event_id', 'event_latitude', 'event_longitude', 'event_depth', 'event_origin_time',
                      'event_magnitude']
    ...
    def _get_Value(self, event, field):
        ...
        if field == 'time':
            ...
            found = 1
        if field == 'error':
            value = [pick.time_errors.uncertainty if pick.time_errors.uncertainty is not None else -100 for pick in
                     event.picks]
