...
### Saving info
directorytosaveto='/Users/benheath/Desktop/Examples/' ### we will save waveforms as well as various pandas structures
...
cat = client.get_events(starttime=starttime, endtime=endtime,minlatitude = latitudeRange[1],maxlatitude =latitudeRange[0],\
...
stations = {'station_latitude':[],'station_longitude':[],'station_elevation':[],'station_name':[],'station_network':[],'station_channel':[]}
...
for inet,network in enumerate(stanet):   
        ...
        for station in network:
                ...
                stations['station_channel'].append(station.channels[0].code[0:2]+'*') # here I am using ??[ZNE] and actually concerned with the ?? part (technical reasons/rarely used )
...
for iarr,_ in enumerate(Arrival['event_id']):
        ...
        for cha in chan:
                ...
                for tr in st:
                        print('Writing '+directorytosaveto+'/'+event_id+'/'+network+'_'+station+'_'+tr.stats.channel+'.'+saveformat)
                        tr.write(directorytosaveto+'/'+event_id+'/'+network+'_'+station+'_'+tr.stats.channel+'.'+saveformat,format = saveformat)
